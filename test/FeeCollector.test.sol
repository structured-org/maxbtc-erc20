...
contract FeeCollectorTest is Test {
    ...
    function testInitializeRevertsOnInvalidCollectionPeriodSecondsTooLow() public {
        FeeCollector impl = new FeeCollector();
        bytes memory data = abi.encodeWithSelector(
            FeeCollector.initialize.selector,
            owner,
    ...
    function testInitializeRevertsOnInvalidCollectionPeriodSecondsTooHigh() public {
        FeeCollector impl = new FeeCollector();
        bytes memory data = abi.encodeWithSelector(
            FeeCollector.initialize.selector,
            owner,
    ...
    function testInitializeRevertsOnStaleExchangeRate() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                FeeCollector.StaleExchangeRate.selector, staleCore.publishedAt(), block.timestamp, 3600
            )
        );
        new ERC1967Proxy(address(impl), data);
    }
    ...
    function testCollectFeeRevertsOnStaleExchangeRate() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(FeeCollector.StaleExchangeRate.selector, rateTimestamp, block.timestamp, 3600)
        );
        feeCollector.collectFee();
    }
    ...
    function testCollectFeeSucceedsWithFreshExchangeRate() public {
        ...
        FeeCollector.State memory st = feeCollector.getState();
        assertEq(st.lastExchangeRate, newRate, "exchange rate should be updated");
    }
    ...
    function testUpdateConfigRevertsOnZeroCoreContract() public {
        vm.expectRevert(FeeCollector.InvalidCoreContractAddress.selector);
        feeCollector.updateConfig(address(0), address(core), 0.2e18, 7200, 3600);
    }

    function testUpdateConfigRevertsOnZeroErReceiverContract() public {
        vm.expectRevert(FeeCollector.InvalidErReceiverAddress.selector);
        feeCollector.updateConfig(address(core), address(0), 0.2e18, 7200, 3600);
    }
    ...
    function testUpdateConfigRevertsOnInvalidFeeReductionTooHigh() public {
        vm.expectRevert(FeeCollector.InvalidFeeReductionPercentage.selector);
        feeCollector.updateConfig(address(core), address(core), ONE, 7200, 3600);
    }

    function testUpdateConfigRevertsOnInvalidCollectionPeriodSecondsTooLow() public {
        vm.expectRevert(FeeCollector.InvalidCollectionPeriodSeconds.selector);
        feeCollector.updateConfig(address(core), address(core), 0.1e18, 60 * 60 - 1, 7200);
    }

    function testUpdateConfigRevertsOnInvalidCollectionPeriodSecondsTooHigh() public {
        vm.expectRevert(FeeCollector.InvalidCollectionPeriodSeconds.selector);
        feeCollector.updateConfig(address(core), address(core), 0.1e18, 60 * 60 * 24 * 30 + 1, 7200);
    }
    ...
    function testUpdateConfigChangesConfig() public {
        ...
        vm.expectEmit(true, true, true, true, address(feeCollector));
        emit FeeCollector.ConfigUpdated(address(newCore), address(core), 0.2e18, 10_000, 7200);

        feeCollector.updateConfig(address(newCore), address(core), 0.2e18, 10_000, 7200);
    ...
    function testUpdateConfigOnlyOwner() public {
        vm.prank(other);
        vm.expectRevert(abi.encodeWithSelector(OwnableUpgradeable.OwnableUnauthorizedAccount.selector, other));
        feeCollector.updateConfig(address(core), address(core), 0.2e18, 7200, 3600);
    }
    ...
    function testProxyReInitializationReverts() public {
        ...
        feeCollector.initialize(owner, address(core), address(core), 0.1e18, 3600, address(maxbtc), 3600);
    }
    ...
    function testUUPSUpgradeOnlyOwner() public {
        ...
        bytes memory data = abi.encodeWithSelector(
            FeeCollector.initialize.selector, owner, address(core), address(core), 0.1e18, 3600, address(maxbtc), 3600
        );
