...
contract WithdrawalManager is Initializable, UUPSUpgradeable, ERC1155HolderUpgradeable, Ownable2StepUpgradeable {
        ...
        returns (bytes4)
    {
        ...
        require(_msgSender() == address(config.withdrawalTokenContract), InvalidWithdrawalToken());
        require(Allowlist(config.allowlistContract).isAddressAllowed(from), AddressNotAllowed(from));
